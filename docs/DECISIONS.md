# CeliacMap — Decisions Log and design detail

> **What this file is.** The long-form memory of the project: every design decision and the reasoning behind
> it, the live-verification notes, the phase-by-phase build status, and the full detail of the open risks.
> It was split out of `CLAUDE.md` on 2026-09-26 because `CLAUDE.md` is loaded into every Claude Code session
> and had grown to about 302,000 characters (the limit is 150,000). `CLAUDE.md` now keeps only an index of
> these entries ("Decisions Log (índice)"), the standing rules ("Reglas vigentes"), the Validator rubric and a
> summary of the architecture and the risks, and links here for anything longer.
>
> **What was moved, and how.** Everything below was copied from `CLAUDE.md` unedited (a script checked
> the copy paragraph by paragraph before the split was committed). The `## Decisions Log` section is the
> original text, byte for byte, including "Build status (phases)". "Key risks to keep in mind",
> "Architecture" and the sections that `CLAUDE.md` now only summarizes are archived after it, under their
> own headings. Only headings and one-line notes were added around the moved text. The one piece of text
> that was **not** copied: the two ```xml prompt bodies of "The Chatbot System Prompts", which were
> byte-identical copies of `supabase/functions/chat/prompts.ts` (checked by script); a note marks where each was.
>
> **How to add to it.** A new decision is a new `###` entry at the end of "Decisions Log" (a new build phase
> goes in "Build status (phases)"). Then add **one index line** to `CLAUDE.md` → "Decisions Log (índice)"
> (title, date, one sentence, link to the entry's anchor) and, only if the decision creates a permanent rule,
> one line to "Reglas vigentes". Anchors are GitHub-style slugs of the heading text: renaming a heading
> breaks the index link that points at it.
>
> **Known quirk, kept on purpose.** The "Key decisions made during development" block (bilingual ES/EN,
> typography, CSS-only map mockup, inline-SVG icons, favicon PNGs) has no heading of its own: it sits at the
> end of the first entry, "Chatbot — named place retrieval". It was not given a heading because the log is
> preserved verbatim; the index in `CLAUDE.md` points at it through that entry.
...
> `CLAUDE.md` keeps a 2–3 line summary of each, with its status, and links here.

### Risk — Secrets boundary

- **Secrets boundary:** never ship the `service_role` key or any API key to the
  browser — only the anon key, made safe by correct RLS.

### Risk — Google Places billing, caps and ToS
...
### Risk — Health-sensitive false approvals
...
### Risk — Search agent stamped city/country from the query target
...
### Risk — resolve_location() can return the wrong business
...
### Risk — VALIDATOR_RESERVE was sized for the old daily cadence
...
### Risk — validation_notes is invisible to the Validator and overwritten
...
### Risk — A redactor failure after a successful insert can duplicate a report
...
## Architecture — texto completo (movido de CLAUDE.md)
...
## Secciones condensadas de CLAUDE.md (texto anterior, verbatim)

> Sections of `CLAUDE.md` that were shortened on 2026-09-26. The previous text is kept here in full.

### Technical Scope (texto anterior)
...
### Design Guidelines (texto anterior)
...
### Suggested Sections (texto anterior)
...
### Documentation Rules (texto anterior)
...
_(the ```xml block that was here is byte-identical to `ROUTER_PROMPT` in `supabase/functions/chat/prompts.ts` and was not copied to this file)_

**Prompt del REDACTOR (llamada 2) — el prompt de sistema del chatbot:**

_(the ```xml block that was here is byte-identical to `RESPONDER_PROMPT` in `supabase/functions/chat/prompts.ts` and was not copied to this file)_
